...
export const addYoutube = (title, url, image) => async(dispatch)=>{
    try {
        ...
        dispatch({
            type: "ADD_YOUTUB_SUCCESS",
            payload: data.message,
        })

    } catch (error) {
        dispatch({
            type: "ADD_YOUTUB_FAILURE",
            payload: error.response.data.message,
        })
    }
}
...
export const deleteProject = (id) => async(dispatch)=>{
    try {
        dispatch({
            type: "DELETE_YPROJECT_REQUEST",
        });

        const {data} = await axios.delete(`/api/v1/admin/project/${id}`);

        dispatch({
            type: "DELETE_YPROJECT_SUCCESS",
            payload: data.message,
        })

    } catch (error) {
        dispatch({
            type: "DELETE_YPROJECT_FAILURE",
            payload: error.response.data.message,
        })
    }
}
